/*
    ...
    note: customization by Thomas P. O'Hara (TPO)
    - shows task entry using small icon
    - based on tips from Grok, Claude, and GPT
    TODO2: redo change with just small icon (to minimize difference from original)
*/
...
PlasmaCore.ToolTipArea {
    ...
    // OLD: height: Math.max(theme.mSize(theme.defaultFont).height, PlasmaCore.Units.iconSizes.medium) + LayoutManager.verticalMargins()
    // TPO: need to squeeze because KDE is being stingy and just using half-height (thus no margin)! */
    // OLD: height: theme.mSize(theme.defaultFont).height

    // GPT: Injected by the view that creates this delegate (TaskList.qml / GroupDialog.qml)
    // OLD: property Item taskList: null

    // GPT: Fill the whole panel row; in popups, keep normal delegate sizing to avoid huge rows.
    implicitHeight: Math.max(theme.mSize(theme.defaultFont).height,
                             PlasmaCore.Units.iconSizes.small) + LayoutManager.verticalMargins()
    
    readonly property real __panelRowHeight: {
        // taskList is available in your context (your trace printed taskList.height/rows),
        // so use it — NOT parent.height.
        if (typeof taskList !== "undefined"
                && taskList
                && taskList.rows !== undefined
                && taskList.rows > 0
                && taskList.height > 0) {
            return Math.round(taskList.height / taskList.rows);
        }
        return implicitHeight;
    }
    
    // BAD GPT: height: inPopup ? implicitHeight : __panelRowHeight
    ...
    PlasmaCore.FrameSvgItem {
	    ...
	    // TODO2: resolve null taskList upstream
            topMargin: (!tasks.vertical && taskList && taskList.rows > 1) ? LayoutManager.iconMargin : 0
            bottomMargin: (!tasks.vertical && taskList && taskList.rows > 1) ? LayoutManager.iconMargin : 0
            leftMargin: ((inPopup || tasks.vertical) && taskList && taskList.columns > 1) ? LayoutManager.iconMargin : 0
            rightMargin: ((inPopup || tasks.vertical) && taskList && taskList.columns > 1) ? LayoutManager.iconMargin : 0
        }
    ...
    Loader {
	    ...
	    // TEST: verticalCenter: parent.verticalCenter   // instead of top/topMargin
        }
	...
	// GPT: Make label.visible depend on a constant icon size, not iconBox.height (to avoid binding loop).
        readonly property int __iconExtent: PlasmaCore.Units.iconSizes.small
        visible: (inPopup || (!iconsOnly && model.IsLauncher !== true
                              && ((parent.width - __iconExtent - PlasmaCore.Units.smallSpacing)
                                  >= (theme.mSize(theme.defaultFont).width * LayoutManager.minimumMColumns()))))
    ...
    readonly property var __view: GridView.view || ListView.view;

    // TODO: var debug = false;
    //
    property bool debug: (
	Qt.application &&
        Qt.application.environment &&
        Qt.application.environment.TPO_DEBUG === "1"
    ) ? true : false

    Component.onCompleted: {
        if (debug) {
            console.warn("TPO Task.qml url=", Qt.resolvedUrl("Task.qml"));
            console.warn("TPO sizes: height=", height,
                         " implicitHeight=", implicitHeight,
                         " view=", __view,
                         " view.cellHeight=", (__view && __view.cellHeight !== undefined) ? __view.cellHeight : "n/a");
    	    console.warn("TPO height=", height,
                         " parent.height=", parent ? parent.height : "n/a",
    			 " taskList=", taskList,
    		         " taskList.height/rows=", taskList ? (taskList.height + "/" + taskList.rows) : "n/a",
    		         " inPopup=", inPopup,
                         " panelRowHeight=", __panelRowHeight,
                         " taskList.height/rows=",
                         (typeof taskList !== "undefined" && taskList) ? (taskList.height + "/" + taskList.rows) : "n/a");
	}
    ...
    onHeightChanged: {
        if (debug) {
	   console.warn("TPO height changed:", height);
	}
    }
    onImplicitHeightChanged: {
        if (debug) {
            console.warn("TPO implicitHeight changed:", implicitHeight);
	}
    }
}
